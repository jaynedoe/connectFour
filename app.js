document.addEventListener("DOMContentLoaded", () => {
  ...
  (function(index){
    ...
    squares[i].addEventListener("click", () => {
      ...
      if(squares[index].classList.contains("available"))
      {
        if (currentPlayer === 1) {
            squares[index].setAttribute("class", "playerOne");
            squares[index - 7].setAttribute("class", "available");
            currentPlayer = 2;
            displayCurrentPlayer.textContent = 2;
    
          } else if (currentPlayer === 2) {
            squares[index].setAttribute("class", "playerTwo");
            squares[index - 7].setAttribute("class", "available");
            currentPlayer = 1;
  ...
  function checkForWin() {
    ...
    for (let i = 0; i < winningArrays.length; i++) {
      const squareOne = winningArrays[i][0];
      const squareTwo = winningArrays[i][1];
      const squareThree = winningArrays[i][2];
      const squareFour = winningArrays[i][3];

      if (
        squares[squareOne].className === "playerOne" &&
        squares[squareTwo].className === "playerOne" &&
        squares[squareThree].className === "playerOne" &&
        squares[squareFour].className === "playerOne"
      ) {
          result.textContent = "Red Won!";
      } else if(
          squares[squareOne].className === "playerTwo" &&
          squares[squareTwo].className === "playerTwo" &&
          squares[squareThree].className === "playerTwo" &&
          squares[squareFour].className === "playerTwo"
      ){
          result.textContent = "Blue Won!"
      }
    }
  }

  squares.forEach(element => element.addEventListener("click", checkForWin));
